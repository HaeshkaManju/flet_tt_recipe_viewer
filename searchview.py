...
class SearchView(ft.UserControl):
    def build(self):
        ...
        self.user_recipe_input = ft.TextField(value="", label="Search Recipes", hint_text="Enter part of a recipe name...")
        recipe_search_button = ft.ElevatedButton(text="Submit", on_click=self.button_recipe_search)
        ########################
        ...
        self.user_ingredients_input = ft.TextField(value="", label="Enter one Ingredient on each line.", multiline=True, min_lines=1, max_lines=6)
        ingredients_search_button = ft.ElevatedButton(text="Submit", on_click=self.button_ingredients_search)        
        
        ###################
        # The Search View #
        ###################
        search_view = ft.Column(width = 600, controls=[ft.Row(controls=[self.header1]), ft.Row(controls=[self.user_recipe_search_label, self.user_recipe_input, recipe_search_button]), ft.Row(controls=[self.user_type_search_label, self.user_type_input, type_search_button]), ft.Row(controls=[self.user_ingredients_search_label, self.user_ingredients_input, ingredients_search_button]) ])
    ...
    def button_recipe_search(self, e):
        # I genuinely do not understand how these buttons work or how the values are getting updated.
    ...
    def button_ingredients_search(self, e):
        self.user_ingredients_search_label.value = self.user_ingredients_input.value
